...
<div class="chat-page-content">
            ...
            <div class="px-4 py-4 border-b border-gray-100 bg-white shrink-0">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div class="relative">
                        ...
                        <div id="actions-menu" class="hidden absolute top-10 right-0 w-44 bg-white border border-gray-100 rounded-xl shadow-lg z-50 py-1">
                ...
                <a href="{{ $chat['route'] }}"
                   ...
                   onclick="openChat(event, '{{ $chat['route'] }}', '{{ $chat['user_id'] ?? '' }}')">
                    ...
                    <div class="relative shrink-0">
                        ...
                        @if($chat['type'] === 'group')
                            <span class="online-dot absolute bottom-0 right-0 w-2.5 h-2.5 bg-purple-400 border-2 border-white rounded-full"></span>
                        @else
                            <span
    ...
    {{-- Personal Chat Modal --}}
    <div id="personal-chat-modal" class="fixed hidden z-50 bg-white rounded-2xl shadow-2xl overflow-hidden w-full max-w-md mx-4"
    ...
    {{-- Group Chat Modal --}}
    <div id="group-chat-modal" class="fixed hidden z-50 bg-white rounded-2xl shadow-2xl overflow-hidden w-full max-w-md mx-4"
         style="top:50%;left:50%;transform:translate(-50%,-50%);max-height:85vh;">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div>
        ...
        <form method="POST" action="{{ route('groups.store') }}" enctype="multipart/form-data" class="flex flex-col" style="max-height: calc(85vh - 80px);">
            @csrf
            <div class="overflow-y-auto flex-1">
                ...
                <div class="px-5 py-4 border-b border-gray-100">
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Add Members (search by email)</label>
                    <div class="relative">
                        <svg class="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                        </svg>
...
<script>
    ...
    // ── Open Chat ──
function openChat(e, url, userId) {        if (e) e.preventDefault();
  ...
  async function performUserSearch(query) {
    ...
    try {
        ...
        users.forEach(user => {
            ...
            userCard.className = 'flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-100 hover:border-[#2D6A4F]/30 hover:bg-[#2D6A4F]/5 transition group cursor-pointer';
            userCard.innerHTML = `
...
function addChatToSidebar(user) {
    ...
    item.href           = chatUrl;
    item.className      = 'chat-item flex items-center gap-3 px-4 py-3.5 hover:bg-gray-50 border-b border-gray-50 transition cursor-pointer';
    ...
    async function searchGroupMembers(query) {
        ...
        try {
            ...
            availableUsers.forEach(user => {
                const userCard = document.createElement('div');
                userCard.className = 'flex items-center gap-3 px-3 py-2 border border-gray-100 rounded-xl hover:border-[#2D6A4F]/30 hover:bg-[#2D6A4F]/5 transition cursor-pointer';
                userCard.innerHTML = `
