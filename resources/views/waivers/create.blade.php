...
@section('content')
    ...
    {{-- Page-Specific Styles for Form Builder --}}
    <style>
        ...
        .tab-btn.active { background: linear-gradient(135deg, #0B3D2E, #2D6A4F); color: white; border-color: transparent; }
        ...
        #fieldCanvas::-webkit-scrollbar { width: 6px; }
        #fieldCanvas::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; }
        #fieldCanvas::-webkit-scrollbar-track { background: transparent; }
    </style>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
                    ...
                    <span>/</span>
                    <span class="text-gray-900 font-medium">Create Waiver</span>
                </div>
...
<input type="text" name="title" value="{{ old('title') }}" placeholder="e.g. Liability Waiver, Membership Agreement..."
            ...
            <div class="bg-white border border-gray-100 shadow-sm rounded-2xl p-6 mb-6">
                ...
                <div class="space-y-3">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="require_signature" value="1" checked class="accent-[#2D6A4F] w-4 h-4">
                        <div>
                    ...
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" checked class="accent-[#2D6A4F] w-4 h-4">
                        <div>
...
<script>
    ...
    var colorMap = {
        blue:   'bg-blue-50 border-blue-200 text-[#0B3D2E]',
    ...
    function addField(type) {
        ...
        if (type === 'fullname' || type === 'email' || type === 'phone') {
            isRequired = true;
        }
    ...
    function renderCanvas(newFieldId) {
        ...
        fields.forEach(function(field) {
            ...
            card.className = 'field-card bg-white border border-gray-200 shadow-sm rounded-lg p-2 mb-2 flex items-start gap-2';            
            ...
            if (['select','radio','checkbox'].includes(field.type)) {
                extra += `<div class="mt-2">
                    ...
                    <input type="text" value="${field.options.join(', ')}"
                        ...
                        class="w-full bg-gray-50 border border-gray-200 text-xs rounded-lg px-3 py-1.5 mt-1 focus:outline-none focus:border-[#2D6A4F] transition"
            ...
            if (field.type === 'rating') {
                extra += `<div class="mt-2">
                    ...
                    <input type="number" value="${field.max || 5}" min="1" max="10"
                        ...
                        class="w-20 bg-gray-50 border border-gray-200 text-xs rounded-lg px-3 py-1.5 mt-1 focus:outline-none focus:border-[#2D6A4F] transition">
                    <div class="text-yellow-400 text-sm mt-1">⭐⭐⭐⭐⭐</div>
                </div>`;
            }
            if (field.type === 'range') {
                extra += `<div class="mt-2 grid grid-cols-3 gap-2">
                    <div><label class="text-xs text-gray-500">Min</label><input type="number" value="${field.min || 0}" oninput="updateMin('${field.id}', this.value)" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-lg px-2 py-1.5 mt-1 focus:outline-none focus:border-[#2D6A4F]"></div>
                    <div><label class="text-xs text-gray-500">Max</label><input type="number" value="${field.max || 100}" oninput="updateMax('${field.id}', this.value)" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-lg px-2 py-1.5 mt-1 focus:outline-none focus:border-[#2D6A4F]"></div>
                    <div><label class="text-xs text-gray-500">Step</label><input type="number" value="${field.step || 1}" oninput="updateStep('${field.id}', this.value)" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-lg px-2 py-1.5 mt-1 focus:outline-none focus:border-[#2D6A4F]"></div>
                </div>
                <div class="mt-1"><input type="range" min="${field.min||0}" max="${field.max||100}" step="${field.step||1}" class="w-full accent-[#2D6A4F]" disabled></div>`;
            }
            if (field.type === 'color') {
                extra += `<div class="mt-2 flex items-center gap-2"><input type="color" value="#2D6A4F" disabled class="w-8 h-8 rounded border border-gray-200 cursor-not-allowed"><span class="text-xs text-gray-400">Color picker preview</span></div>`;
            }
    ...
    function updateFieldCount() {
        ...
        b.className = fields.length > 0
            ? 'bg-green-50 border border-green-200 text-[#2D6A4F] text-xs px-3 py-1 rounded-full font-medium'
